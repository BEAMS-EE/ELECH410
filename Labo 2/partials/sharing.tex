...
\E{
	...
	\begin{itemize}
		...
		\item Using a watch window and the debugger, check the value of \kw{cntr}. Is this the expected result? Why? (Use the logic analyser to help. Note that GPIO J14 is toggled from low to high at the beginning of the \kw{main} function so that you can trigger the logic analyzer to the beginning of the execution. )
	\end{itemize}
}{}\reponse{
...
The \textbf{mutex} (see chapter over priority-driven systems) is a convenient way to protect shared data, while still handling interrupts and authorise the preemption of the running task. Note: when a task acquires a resource via \kw{xSemaphoreTake()}, it should not forget to release it via \kw{xSemaphoreGive()}. A pseudo code is given in Listing \ref{lst:listing 1}.
